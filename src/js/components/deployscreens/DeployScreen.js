...
import AddHostByBMC from "./AddHostByBMC";
import AddHostByOV from "./AddHostByOV";
...
class DeployScreen extends Component {
  ...
  getKickStartsForOSPackage(osPackage){
    ...
    console.log("getKickStartsForOSPackage: this.props.screenData.osPackages: ", this.props.screenData.osPackages);

    let package1 = this.props.screenData.osPackages.filter(item => item.package === osPackage);
    ...
    if (this.state.addhost === true) {
      ...
      if((this.props.deployServerSettings['deploymentMode'] === "hpeilo5") ||
        ...
        addHostControls =
            ...
            <AddHostByOV
                deployServerSettings={this.props.deployServerSettings}
                serverProfiles={this.props.serverProfiles}
                heading={heading}
                handleAddHost={this.handleAddHost}
                handleCancelAddHost={this.handleCancelAddHost}
            />
  ...
  render() {
    ...
    let ospPlaceHolderText = (screenData.osPackages.length >= 1)? "Select":"None";
    if(screenData.osPackages) {
      ospList = screenData['osPackages'].map((item, index) => (
        item.package
      ));
    }
    ...
    return (
      ...
      >
        ...
        <Box gridArea='method-fields' justify='start' >
          {this.props.children}
        </Box>
        <Box gridArea='os-fields' justify='start' >
        ...
        <Box gridArea='common-fields' justify='start' >
        ...
        <Box gridArea='hosts' justify='start' >
          <Form>
