...
import {getRESTApi, deleteRESTApi, postImage, postBinFile} from '../api/server-rest';
...
export function deleteImage (query, file) {
  ...
  return dispatch => {
    ...
    deleteRESTApi(uri, (err, res) => {
      ...
      dispatch(deleteImage_Success({'result': res, 'error': err}));
    });


      // .then((response) => {
      //   console.log("deleteImage: response: ", response);
      //   dispatch(deleteImage_Success(response.result));
      // })
      // .catch((err) => {
      //   console.log("deleteImage: err: ", err);
      // })

    // deleteRESTApi(uri, function(result){
    //   console.log("action deleteImage : result: ", (result));
    //   dispatch(deleteImage_Success(result));
    // });
